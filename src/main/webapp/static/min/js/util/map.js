...
fw.browser = {
	...
	version : navigator.userAgent.match(/(?:msie|firefox|opera|Chrome|Safari)[ |\/]([\d\.]+)/i)[1]
};
...
fw.ajax.request = function (json){
	...
	fw.isff && xmlHTTP.overrideMimeType("text/html;charset=GBK");
...
//秒转换为倒计时显示
fw.conv.rtForShow = function (rt){
	var hour = 	Math.floor(parseInt(rt / 60 / 60));
	var second = Math.floor(rt % 60);
	var minute = Math.floor((rt - second) / 60 - (hour*60));			
...
//秒转换为倒计时显示
fw.conv.rtForShow2 = function (rt){
	var hour = 	Math.floor(parseInt(rt / 60 / 60));
	var second = Math.floor(rt % 60);
	var minute = Math.floor((rt - second) / 60 - (hour*60));
...
//比较是否同一天(a , b为日期字符串 比如格式:yyyy-MM-dd hh:mm)
fw.date.compareDay = function (a,b){
	 var d1 = fw.date.format(a);
	 var d2 = fw.date.format(b);
...
fw.date.getCurrentDateByYYYYMMDD = function (){
	var date = new Date();
...
fw.dom.setObjClass = function (obj, value) {
	obj.className = value;
}
...
//设置Display样式
fw.dom.setStyleDisplay = function(obj,st){
...
//冒泡 对像排序
fw.math.objcetBubbleSort = function(arr,attribe){
	for (var i=arr.length-1;i>0;i--){
		for (var j=0;j<i;j++){
...
fw.re.mobile = function (condition) {
    return fw.re.parse(/1(?:3\d|4\d|5\d|7\d|8\d)-?\d{8}/, condition);
}
...
//过滤数组 table.rows[2].childNodes
fw.ieff.filterArr = function (arr){
	var tempArr = new Array();
	for(var v=0;v<arr.length;v=v+1){
		if(arr[v].nodeType == 1){
			tempArr.push(arr[v]);
		}
	}
...
//过滤HTML标签  解决innerHTML
fw.ieff.filterInnerHTML = function (obj){
	return obj.innerHTML.replace(/<.+?>/gim,'').replace(/(^\s*)|(\s*$)/g, "");
}
...
function keepSession()
{
	...
	var r = fw.ajax.callFile(dataUrl);
	//alert(r);
	window.setTimeout(keepSession, 2*60*1000);
}
/**
...
if (this.elements[i].key.toString() == _key.toString()) {
...
try {
for (i = 0; i < this.elements.length; i++) {
if (this.elements[i].value.toString() == _value.toString()){
bln = true;
}
}
} catch(e) {
