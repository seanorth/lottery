...
import com.cosun.entities.Order;
...
public class OrderController {
	...
	/*	String 双色球单式="DS\\.((\\d{2}),){5}(\\d{2})\\|(\\d{2})\\.false";
		...
		*/
		String loty=order.getGameCode();
		String strs=order.getNumber();
		//String[] split = strs.split("#");
		String[] split = strs.split("~");
		//假设split只能为单式数组或其他式一注（split[0]）
		String  str=split[0];
		List<Lottery> list= lotteryService.findByGameCode(loty);
		Lottery l=list.get(0);
		
		for (Pattn p : l.getPattns()) {
